...
test("maps unsupported score policies to a stable failure code", () => {
  const failure = safeFailureDetails(
    new Error("UNSUPPORTED_METHODOLOGY"),
    "configured-model",
    "EVIDENCE_ASSESSMENT",
  );

  assert.equal(failure.code, "UNSUPPORTED_METHODOLOGY");
  assert.equal(failure.metadata.retryable, false);
});
