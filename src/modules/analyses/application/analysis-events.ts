...
export function safeFailureDetails(
  ...
  if (error instanceof AIAnalyzerError) {
    const messages: Record<AIAnalyzerError["code"], string> = {
      ...
      PROVIDER_INVALID_REQUEST:
        "O provedor de IA recusou a configuração da solicitação.",
  ...
  if (error instanceof Error && error.message === "UNSUPPORTED_METHODOLOGY") {
    return {
      code: error.message,
      publicMessage: "A versão da metodologia desta análise não é suportada.",
      technicalMessage: "Analysis precondition failed: UNSUPPORTED_METHODOLOGY",
      metadata: { retryable: false },
      stage,
    };
  }
