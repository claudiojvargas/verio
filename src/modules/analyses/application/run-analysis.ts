...
import { auditDigitalChannels } from "@/modules/analyses/application/digital-channel-audit";
...
import { VERIO_SCORE_V2 } from "@/modules/scoring/policies/verio-score-v2";
...
export async function runAnalysis(analysisId: string) {
  ...
  try {
    const policy = resolveScorePolicy(analysis.methodologyVersion);
    ...
    const registeredAssessment = assessRegisteredChannels(
      analysis.business.channels,
    );
    if (registeredAssessment.evidence.length === 0)
    ...
    await recordAnalysisEvent(db, {
      ...
      metadata: {
        evidenceCount: registeredAssessment.evidence.length,
        competitorCount: analysis.competitors.length,
      },
      durationMs: Date.now() - evidenceStartedAt,
    });

    let primaryAssessment = registeredAssessment;
    if (policy.version === VERIO_SCORE_V2.version) {
      currentStage = "DIGITAL_CHANNEL_AUDIT";
      const auditStartedAt = Date.now();
      await recordAnalysisEvent(db, {
        analysisId: analysis.id,
        stage: "DIGITAL_CHANNEL_AUDIT",
        status: "RUNNING",
        publicMessage: "Auditando site e estrutura do WhatsApp.",
      });
      const audit = await auditDigitalChannels(analysis.business.channels);
      primaryAssessment = {
        evidence: [...registeredAssessment.evidence, ...audit.evidence],
        observations: mergeObservations(
          registeredAssessment.observations,
          audit.observations,
        ),
      };
      await recordAnalysisEvent(db, {
        ...
        stage: "DIGITAL_CHANNEL_AUDIT",
        status: "COMPLETED",
        publicMessage: "Auditoria técnica dos canais concluída.",
        technicalMessage:
          "Public website foundations and WhatsApp structure assessed",
    ...
    const score = calculateVerioScore(primaryAssessment.observations, policy);
    await recordAnalysisEvent(db, {
      analysisId: analysis.id,
      stage: "SCORE_CALCULATION",
      status: "COMPLETED",
    ...
    const competitorResults = await Promise.all(
      analysis.competitors.map(async (competitor) => {
        const channels = parseCompetitorChannelsSnapshot(
          competitor.channelsSnapshot,
        );
        const registered = assessRegisteredChannels(channels);
        const audit =
          policy.version === VERIO_SCORE_V2.version
            ? await auditDigitalChannels(channels)
            : null;
        const assessment = {
          evidence: [...registered.evidence, ...(audit?.evidence ?? [])],
          observations: mergeObservations(
        ...
        return {
          ...
          assessment,
          score: calculateVerioScore(assessment.observations, policy),
        };
      }),
    );
    await recordAnalysisEvent(db, {
...
function mergeObservations(
  baseline: readonly ScoreObservation[],
  collected: readonly ScoreObservation[],
) {
  return [
    ...new Map(
      [...baseline, ...collected].map((observation) => [
...
function resolveScorePolicy(version: string): ScorePolicy {
  if (version === VERIO_SCORE_V1.version) return VERIO_SCORE_V1;
  if (version === VERIO_SCORE_V2.version) return VERIO_SCORE_V2;
  throw new Error("UNSUPPORTED_METHODOLOGY");
}
