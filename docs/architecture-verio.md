...
### 16.1.1 Empacotamento em containers

O empacotamento Docker mantém a mesma topologia lógica. O `Dockerfile` usa
multi-stage build e output standalone do Next.js; o runtime é não-root. Para
desenvolvimento e validação local, `compose.yaml` coordena PostgreSQL, uma etapa
one-shot de migrations e a aplicação. O seed é um profile manual, nunca uma etapa
automática de inicialização.

Containers são uma forma de distribuição, não um motivo para separar módulos em
serviços. Produção pode usar a imagem ou um runtime gerenciado equivalente. Banco
continua externo/gerenciado em produção; o PostgreSQL do Compose é local.
