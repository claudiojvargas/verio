...
Localmente, a topologia pode ser executada por Docker Compose com containers
separados para aplicação, migration one-shot e PostgreSQL. Isso não transforma o
monólito em microsserviços: existe um único processo de aplicação.
...
### Timeline e depuração de análises

Cada job persiste eventos sanitizados por etapa em `AnalysisEvent`. A página da
análise consulta esses eventos respeitando ownership e atualiza a timeline por
polling enquanto o processamento estiver ativo. O painel técnico pode mostrar
código, provider, modelo, tentativa e duração, mas nunca credenciais, headers,
prompt, resposta bruta ou stack trace. Falhas do provider preservam um código
estável e uma mensagem acionável sem expor o objeto de erro original.
...
- A migration roda antes da aplicação; seed é sempre explícito e somente local.
- O container de runtime executa como usuário não-root e usa output standalone.
