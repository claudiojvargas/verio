...
Com Docker Compose v2 e `curl` instalados, execute:

```bash
./scripts/install.sh
# ou: npm run install:interactive
```

O instalador solicita sem ecoar no terminal apenas as credenciais do Clerk e do
Google AI. Antes de escrever `.env`, ele verifica:

1. preenchimento e prefixos esperados (`pk_test_`/`pk_live_`,
   `sk_test_`/`sk_live_` e `AIza`);
2. se as duas chaves Clerk pertencem ao mesmo ambiente;
3. existência da instância identificada pela publishable key e autenticação real
   da secret key na API do Clerk;
4. acesso real à listagem de modelos da Google Generative Language API.

Uma falha de rede ou autenticação interrompe a instalação sem persistir as novas
chaves. Se `.env` já existir, o instalador cria um backup timestamped, preserva as
demais variáveis e substitui somente as três credenciais. O arquivo resultante usa
permissão `600`. Depois, o script valida o Compose e executa
`docker compose up --build --detach --wait`.

> A validação envia cada segredo somente ao endpoint HTTPS oficial do respectivo
> fornecedor. As chaves não são impressas nem passadas como argumento de processo.

### Instalação manual

O Compose oferece quatro serviços padrão:
...
| `scheduler` | Aciona o processador interno de análises pendentes |
